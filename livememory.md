...
## 0. Strategic Base For MrQuentilha (2026-03-07)
- Decision: `HexAgentGUI` is now the architecture seed for `MrQuentilha`.
- New base contract registered in `docs/mrquentilha_base_blueprint.md`.
- The base must evolve in an `Antigravity IDE` compatible mode: artifact-driven, resumable, workspace-first.
- The base is defined through four axes:
  - structure: `frontend -> controllers -> services -> core -> integrations`
  - memory: runtime, session, operational, long-term AI, profile, architectural, governance
  - governance: living docs + roadmap + audit + tests as gates
  - planned evolution: Phase 0 alignment before deeper product specialization
- Antigravity alignment rule:
  - implementation artifacts like `implementation_plan.md` are part of the delivery contract, not optional notes
  - architectural decisions must remain recoverable from repo files
  - core logic must not depend directly on IDE vendor APIs
- Immediate Codex track:
  - normalize profile and memory contracts
  - reduce orchestration monolith size
  - expand validation gates in controllers/services/core
  - consolidate the Antigravity artifact protocol for resumed sessions
...
✅ **Revisão API Nativa:** Painéis HexStrike nativos (Ferramentas, BugBounty, CTF, Processos) migrados para Service Provider (`HexStrikeService.js`), abandonando chamadas abertas e não documentadas de `APIClient`.
...
1. **Dashboard HexStrike Visual:** O frontend iniciará a conversão das rotas de Payload Nativo (ex: `POST /api/bugbounty/reconnaissance-workflow`) para abas interativas completas. (CONCLUÍDO)
2. **Terminal Híbrido Co-Pilot:** Permitir que o modelo (Linter Assistivo) auto-corrija as flags digitadas passivamente pelo humano na linha de comando antes do *Enter*.
3. **Ecossistema MCP:** Planejar a GUI para dar attach em novos servidores, como o `mcp-kali-server` e o substituto do plugin FileSystem herdando regras de MCP.
4. **Monitoramento Passivo (Daemon):** O frontend (SystemMetrics ou Daemon via Context) consumindo `/api/telemetry` para detecções dinâmicas e deploy de honeypots guiados.
