...
### Added / Adicionado
- **🔥 HexStrike Integration** / **Integração HexStrike**
  - Full integration with HexStrike AI execution engine
  - MCP (Model Context Protocol) server for command execution
  - Unified VENV management for all components
  - Auto-start service logic in Backend
  
- **🧠 AgentCore Evolution** / **Evolução do AgentCore**
  - Dynamic status synchronization (`refresh_status`)
  - Auto-healing connection logic for MCP
  - Race condition fix for service startup

### Fixed / Corrigido
- **🐛 Critical Fixes** / **Correções Críticas**
  - Fixed "Process died during startup" in HexStrike
  - Resolved `hexstrike_available=False` stuck state
  - Fixed VENV path inconsistencies in `install.sh` vs `system_config`
