...
> Strategic extension (2026-03-07): this architecture is now the reference base for `MrQuentilha`.
> See `docs/mrquentilha_base_blueprint.md` for the structural, memory, governance, and evolution contract derived from `HexAgentGUI`.
> This base is also aligned for artifact-driven evolution via `Antigravity IDE`.
...
4.  **`ShellBlock`**:
    ...
    - *Func:* `xterm.js` instance connected to local PTY. Real-time ZSH emulation.
5.  **`ToolCallBlock`**:
    - *State:* Running / Completed / Errored.
    - *Func:* Specialized block explicitly intercepting HexStrike Client native endpoints or external MCP tools, bypassing raw terminal stdout, resolving cognitive dissonance gracefully.
6.  **`NarrativeBlock`**:
...
**Version:** 2.2.0 (Cognitive Resonance, Native ToolCalls & Hybrid Persistence)
