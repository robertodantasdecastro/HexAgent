================================== REVISÃO DE PROJETO ======================================
### Prompt: Auditoria Holística, Refatoração POO e Evolução de Interface Reativa (HexAgentGUI)

**Objetivo / Objective:**
Realizar uma análise profunda e exaustiva do projeto HexAgentGUI para sincronizar o estado atual com o plano de desenvolvimento de longo prazo. O foco é garantir padrões rigorosos de POO, escalabilidade extrema, e a sincronização total de todas as variáveis da interface GUI com o backend e o ecossistema HexStrike-AI.
/ Perform a deep and exhaustive analysis of the HexAgentGUI project to synchronize the current state with the long-term development plan. The focus is to ensure rigorous OOP standards, extreme scalability, and total synchronization of all GUI variables with the backend and the HexStrike-AI ecosystem.

**Instruções Detalhadas / Detailed Instructions:**

1.  **Mapeamento e Sincronização Global de Variáveis (Alta Prioridade):**
    *   Identifique e mapeie todas as variáveis na interface GUI e arquivos de configuração localizados em `~/.hexagent-gui/`.
    *   Escaneie recursivamente todas as pastas em busca de arquivos de estado.
    *   Garanta a inicialização correta e o encapsulamento POO, assegurando que cada módulo backend reflita o estado da UI em tempo real.

2.  **Auditoria POO e Refatoração de Código Legado:**
    *   Identifique lógica procedural remanescente e converta-a em classes modulares e extensíveis.
    *   Analise dependências entre componentes para eliminar acoplamento rígido.

3.  **Análise de Motores de IA e Comunicação Dinâmica:**
    *   Avalie a integração com motores online (OpenRouter, ChatGPT, DeepSeek, Claude) e offline (LM Studio, 5ire).
    *   Otimize a comunicação via MCP (Model Context Protocol) para plugins de IDE (Cursor, Antigravity).

4.  **Integração Profunda com HexStrike-AI:**
    *   Verifique o vínculo com a aplicação HexStrike-AI e seu suporte MCP para ferramentas Kali Linux.
    *   Refine os canais de controle para que a IA possa executar comandos no terminal, processar o retorno e decidir o próximo passo autonomamente.

5.  **Arquitetura de Blocos de Inferência (Interface Cyberpunk):**
    *   Refatore o chat para um sistema de blocos modulares sequenciais:
        *   **Input/Inferência:** Edição aborta execução atual; novos prompts durante execução tornam-se diretrizes dinâmicas.
        *   **Processamento Cognitivo (Thinking):** Painel minimalista "Kernel Debug" com toggle para *Chain of Thought*.
        *   **Resposta Narrativa:** Explicação estratégica e técnica.
        *   **HexStrike Shell:** Emulação ZSH em tempo real com syntax highlighting e controles de execução.
        *   **Análise Pós-Execução:** Feedback loop automático para definir o status da missão.

6.  **Sincronização de Fluxo e Automação:**
    *   Vincule o botão "Auto-Execute" à transição automática entre blocos de explicação e terminal.
    *   Implemente o "Safety Trigger" (Limitador de Ciclos) para prevenir loops infinitos, exigindo autorização humana para "Modo Infinito".

7.  **Workflows Especializados e Templates de Missão:**
    *   Crie templates de contexto para Pentest, OSINT, Engenharia Reversa, etc.
    *   Orquestre ferramentas gráficas e de terminal via MCP conforme o cenário.

8.  **Documentação e Limpeza:**
    *   Mantenha o padrão Inglês (Primário) / Português-BR (Secundário).
    *   Remova redundâncias, funções duplicadas e variáveis obsoletas.

**Formato de Saída / Output Format:**

*   **Resumo das Descobertas (Findings Summary):** Status das variáveis, progresso POO e bugs críticos.
*   **Plano de Desenvolvimento Atualizado (Updated Roadmap):** Cronograma detalhado de tarefas secundárias, testes e operação.
*   **Documentação de Arquitetura (Architecture Doc):** Árvore de arquivos, Diagrama de Classes e Stack de dependências atualizada.
