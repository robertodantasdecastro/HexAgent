...
**Status:** ⚠️ **Good but Needs Evolution / Bom mas Precisa Evoluir**
- **Connection:** `HexStrikeClient` successfully communicates with port 8888.
- **Execution:** Basics are in place (`execute_command`, `execute_tool`).
- **Gap:** The "Cyberpunk Interface" requires faster, real-time feedback. Currently, it relies on HTTP request/response cycles. Using WebSockets or SSE (already in `ChatController`) is good, but the UI needs to visualize this better as "Blocks" (Thinking, Shell, Analysis).

## 4. Inference Blocks Architecture / Arquitetura de Blocos de Inferência

**Status:** ❌ **Partial / Parcial (Implementation Gap)**
- **Current State:** `SmartBlock.jsx` exists and handles `THINKING` detection via regex, but it is reactive, not structural. 
...
- **Requirement:** Refactor `ChatController` to emit strict "Block Start" / "Block End" events, and `ChatContainer` to manage a list of specific Block Components (`<InputBlock />`, `<ThinkingBlock />`, `<ShellBlock />`) instead of a generic list of strings.
