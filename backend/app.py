...
from services.hexstrike_manager import HexStrikeManager # Import for Auto-Start
...
def create_app(core_ref=None, hexstrike_ref=None):
    ...
    log_dir = os.path.expanduser('~/.hexagent-gui/logs')
    ...
    if agent_core is None:
        try:
            ...
            agent_core = AgentCore(
                ...
                model=model,
                system_prompt=system_prompt,  # Inject System Prompt
            ...
            # Auto-Start HexStrike Service if not running (SKIP IN SETUP MODE)
            # Auto-Iniciar Serviço HexStrike se não estiver rodando (PULAR EM MODO SETUP)
            if not os.environ.get('HEXAGENT_SETUP_ONLY'):
                try:
                    hex_manager = HexStrikeManager()
                    if not hex_manager.is_running():
                        app.logger.info("⚡ Auto-Starting HexStrike Service...")
                        success, msg = hex_manager.start()
                        if success:
                            app.logger.info(f"   ✓ HexStrike Service Started: {msg}")
                            # Update AgentCore status
                            agent_core.refresh_status()
                            # Reload MCP to connect to the new server
                            agent_core.reload_mcp()
                        else:
                            app.logger.error(f"   ✗ HexStrike Auto-Start Failed: {msg}")
                    else:
                        app.logger.info("   ✓ HexStrike Service already running")
                except Exception as e:
                    app.logger.error(f"   ⚠ HexStrike Auto-Start Error: {e}")
            else:
                 app.logger.info("⚠️ Skipping HexStrike Auto-Start (Setup Mode)")
