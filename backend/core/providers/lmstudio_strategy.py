...
class LMStudioStrategy(InferenceStrategy):
    ...
    CHAT_MODELS = [
        ...
        'qwen', 'yi', 'mixtral', 'solar', 'orca', 'nous'
    ]
    ...
    def _is_chat_model(self, model_name: str) -> bool:
        ...
        for comp_model in self.COMPLETION_MODELS:
            if comp_model.lower() in model_lower:
                logger.info(f"[LMStudio] Detected completion model: {model_name}")
                return False
        ...
        for chat_model in self.CHAT_MODELS:
            if chat_model.lower() in model_lower:
                logger.info(f"[LMStudio] Detected chat model: {model_name}")
                return True
        
        # Default: assume chat model (safer, more common)
        # Padrão: assumir modelo de chat (mais seguro, mais comum)
        logger.warning(f"[LMStudio] Unknown model type: {model_name}, defaulting to chat API")
        return True
    ...
    def chat_step(self, prompt: str, chat_context: Optional[List[Dict[str, str]]] = None, model: Optional[str] = None) -> Generator[str, None, None]:
        ...
        try:
            logger.info(f"[LMStudio] Sending request: model={model_to_use}, endpoint={'chat' if is_chat else 'completion'}, history={len(chat_context) if chat_context else 0} msgs")
