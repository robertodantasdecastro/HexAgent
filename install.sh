...
detect_hexstrike_location() {
    ...
    # Install HexStrike-AI Python dependencies into the unified venv
    # Instala dependências Python do HexStrike-AI no venv unificado
    # BUG C FIX: aiohttp and other hexstrike-ai deps must be in the unified venv
    # CORREÇÃO BUG C: aiohttp e outras deps devem estar no venv unificado
    if [ -n "$HEXSTRIKE_PATH" ] && [ -f "$HEXSTRIKE_PATH/requirements.txt" ] && [ -f "venv/bin/pip" ]; then
        print_info "Installing HexStrike-AI dependencies into unified venv... / Instalando dependências do HexStrike-AI..."
        # Install only the lightweight deps that are safe for the unified venv (aiohttp, flask, requests, psutil, fastmcp)
        # Heavy security tools (pwntools, angr, mitmproxy) are skipped — these require system-level setup
        HEXSTRIKE_CORE_DEPS="aiohttp>=3.8.0,<4.0.0 flask>=2.3.0,<4.0.0 requests>=2.31.0,<3.0.0 psutil>=5.9.0,<6.0.0 fastmcp>=0.2.0,<1.0.0 beautifulsoup4>=4.12.0,<5.0.0"
        venv/bin/pip install --quiet $HEXSTRIKE_CORE_DEPS 2>&1 | tail -3
        if [ $? -eq 0 ]; then
            print_success "HexStrike-AI core dependencies installed in unified venv"
        else
            print_warning "Some HexStrike-AI dependencies may have failed to install (non-critical)"
        fi
    fi
}
...
setup_user_config() {
    ...
    # 6. Install/Update mcp-config.json (MCP Registry)
    ...
    # Sincroniza diretório de agentes (personas, configs de agente)
    if [ -d "config_templates/agents" ]; then
        print_info "Syncing agent personas / Sincronizando personas dos agentes..."
        mkdir -p "$USER_CONFIG_DIR/agents"
        for agent_file in config_templates/agents/*.json; do
            fname=$(basename "$agent_file")
            dest="$USER_CONFIG_DIR/agents/$fname"
            if [ ! -f "$dest" ]; then
                # First install: copy the template / Primeira instalação: copia o template
                cp "$agent_file" "$dest"
                print_success "Installed agent config: $fname"
            else
                # Already exists: merge new keys only (preserve user changes)
                # Já existe: faz merge apenas das novas chaves (preserve edições do usuário)
                python3 scripts/verify_config.py "$dest" "$agent_file"
                print_success "Updated agent config (merged): $fname"
            fi
        done
    fi

    # 8. Create scripts/ directory for Script Builder (Phase C4)
    # Cria diretório scripts/ para o Script Builder (Fase C4)
    mkdir -p "$USER_CONFIG_DIR/scripts"
    print_success "Scripts directory ready: $USER_CONFIG_DIR/scripts"

    # 9. Cleanup Legacy 'config/' folder if it exists
...
setup_configs() {
    ...
    # Skip running app.py during install to prevent service startup
    # Pula execução do app.py durante instalação para evitar início de serviços
}
...
build_app() {
    ...
    # Ensure config templates are copied
    # Garante que templates de config sejam copiados
    mkdir -p "$INSTALL_DIR/config_templates"
    cp -r config_templates/* "$INSTALL_DIR/config_templates/"
...
create_shortcuts() {
    ...
    # Ensure target directories exist / Garante que os diretórios de destino existam
...
analyze_temp_files() {
    ...
    # Auto-select option 3 for non-interactive background execution / Auto-selecionar opção 3
    echo -e "${YELLOW}Non-interactive mode: Auto-selecting option 3 (Keep files)${NC}"
    choice="3"
