...
const ShellBlock = ({ content, metadata = {}, status = 'active' }) => {
  ...
  // Optimized Delta Update Logic
  // Lógica de Atualização Delta Otimizada
  useEffect(() => {
    if (!xtermInstance.current) return;
    ...
    // Detect reset (content shorter than before) or full reload
    // Detectar reset (conteúdo menor que antes) ou recarga completa
    if (currentLength < lastLength) {
        ...
        // Anexar apenas novo conteúdo
        const delta = content.slice(lastLength);
        term.write(delta);
        lastContentLengthRef.current = currentLength;
    }
    
    // Scroll to bottom if content changed
    // Rolar para o fundo se conteúdo mudou
    if (currentLength !== lastLength) {
        term.scrollToBottom();
        // Re-fit occasionally or just on resize is usually enough, but here ensures fit
        fitAddon.current?.fit();
    }
