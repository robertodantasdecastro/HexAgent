  ...
  async load() {
    try {
      ...
      if (result.success && result.data) {
        // Store full config object if available
        // Armazenar objeto de config completo se disponível
        this.config = result.data.config || result.data;
        
        // Read from top-level response fields (returned by backend GET /config/ai)
        // CORRIGIDO: antes lia result.data.config.ai.model que não existia → model=undefined
        const hasKey = result.data.has_api_key !== undefined
          ? result.data.has_api_key
          : !!this.config?.ai?.api_key;
        const model = result.data.model || result.data.engine || this.config?.ai?.model;
        const engine = result.data.engine || this.config?.engine;
        
        console.log(`[AIConfigManager] Loaded engine=${engine}, model=${model}, has_api_key=${hasKey}`);
