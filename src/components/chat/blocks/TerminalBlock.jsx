...
const TerminalBlock = ({ command, output, exitCode, autoExecute = false, isExecuting = false }) => {
  ...
  return (
    <div className="my-4 rounded-lg overflow-hidden border border-gray-700 bg-[#0d1117] shadow-xl font-mono text-sm max-w-full">
      ...
      {/* Execution Status */}
      {isExecuting && (
         <div className="px-3 py-2 bg-blue-900/10 text-blue-300 text-[10px] border-t border-blue-900/30 flex flex-col gap-2">
            <div className="flex justify-between items-center">
               <span className="flex items-center gap-2 animate-pulse">
                  <span className="w-2 h-2 rounded-full bg-blue-400"></span>
                  Executing...
               </span>
               <span className="font-mono opacity-80">{elapsed}s</span>
            </div>
            <div className="w-full h-1 bg-blue-900/30 rounded overflow-hidden">
               <div className="h-full bg-blue-500/50 w-full rounded animate-pulse"></div>
            </div>
         </div>
      )}
    </div>
