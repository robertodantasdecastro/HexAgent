...
const ThinkingBlock = ({ content }) => {
  ...
  return (
    <div className="my-2 border border-gray-700 rounded-lg bg-[#0d1117] overflow-hidden">
      ...
      >
        {isCollapsed ? <ChevronRight size={16} className="text-gray-400 flex-none" /> : <ChevronDown size={16} className="text-gray-400 flex-none" />}
        <Brain size={16} className="text-purple-400 flex-none" />
        <span className="text-xs font-mono text-gray-400 select-none flex-1 truncate">
          {isCollapsed && content ? 
            content.replace(/\n/g, ' ').substring(0, 50) + '...' : 
