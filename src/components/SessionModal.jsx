...
const SessionModal = ({ isOpen, onClose, onLoadSession, onSaveSession, currentSessionName, onClearSession, onDeleteSession }) => {
                ...
                {loading ? (
                    ...
                    <div className="space-y-2">
                        {sessions.map(session => (
                            <div key={session.name} className="group flex items-center justify-between p-3 rounded bg-[#111] border border-[#222] hover:border-cyan-500/30 hover:bg-[#151515] transition-all">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-[10px] text-gray-600 font-mono">{new Date(session.modified * 1000).toLocaleString()}</span>
                                    </div>
                                </div>
                ...
                <div>
                     ...
                     <div className="space-y-4">
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={handleSave}
                                className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded transition-colors text-sm font-medium"
                            >
                                <Save size={16} />
                                {t('session.save', 'Save Current')}
                            </button>
                            
                            <button
                                onClick={() => {
                                    if (window.confirm(t('session.confirm_clear', 'Are you sure you want to clear the current session?'))) {
                                        onClearSession && onClearSession();
                                    }
                                }}
                                className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-red-900/40 hover:bg-red-900/60 text-red-200 border border-red-900/50 rounded transition-colors text-sm font-medium"
                            >
                                <Trash2 size={16} />
                                {t('session.clear', 'Clear Session')}
                            </button>
                        </div>
                    </div>
                </div>
